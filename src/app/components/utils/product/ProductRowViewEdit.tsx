...
interface ProductRowViewProps {
  item: product | null;
  onDeleted: (e: product) => void;
}

const ProductRowViewEdit: React.FC<ProductRowViewProps> = ({ item, onDeleted }) => {
  ...
  return (
    <div
      className="min-h-[70px] cursor-pointer border-2 border-transparent w-full flex flex-row flex-nowrap gap-2 items-center justify-between px-[5px] bg-gray-200 hover:border-2 hover:border-gray-300 rounded-sm shadow-sm ease-in-out"
    >
      ...
      <div className="flex flex-row gap-6 items-center justify-center min-h-[65px]">
        ...
        <button
          className="bg-gray-400/80 border-gray-400 border-3 text-white px-[4px] rounded cursor-pointer min-h-[70px]"
          onClick={() => {
            if (item) onDeleted(item);
          }}
        >
...
export default ProductRowViewEdit;
